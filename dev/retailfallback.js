...
(async () => {
  ...
  for (const mode of ['reduced','no-js']) {
    for (const [width,height] of [[1440,900],[390,844]]) {
      ...
      const ok = result.poster && result.canvasHidden && result.titleVisible && result.ctaVisible && result.overflow <= 1 &&
        (mode === 'no-js' || result.reducedClass);
      if (!ok) failures++;
      console.log(`${mode} ${width}x${height} ${ok?'PASS':'FAIL'} poster=${result.poster} canvasHidden=${result.canvasHidden} title=${result.titleVisible} cta=${result.ctaVisible} overflow=${result.overflow}`);
      await page.close();
    }
  }
  await browser.close();
