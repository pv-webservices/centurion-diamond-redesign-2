/* frame cost while scrubbing 06 · Collection, and specifically the aperture close */
...
(async () => {
  ...
  const runs = await p.evaluate(async () => {
    ...
    const rail    = stats(await scrub(0.21, 0.85, 160));
    const aperture= stats(await scrub(0.00, 0.14, 70));
    return { whole, rail, aperture };
  });
