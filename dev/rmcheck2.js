...
(async()=>{
 ...
 for(const [w,h] of [[390,844],[1440,900]]){
  ...
  const r=await p.evaluate(()=>{
    ...
    const hidden=[], sel='.plr__t,.plr__b,.plr__h2,.cl__cut,.cl__sku,.cl__spec,.wrn__t,.wrn__b,.wrn__h2,.dsp__t,.dsp__b,.dsp__h2,.cscene__title,.cscene__body,.wrn__plate img,.dsp__case img,.cl__stone img,.fin__title,.fin__body,.fin__case img,.fin__cta';
    ...
    const pinned=[...document.querySelectorAll('.chero__stage,.plr__stage,.coll__stage,.wrn__stage,.dsp__stage,.fin__stage')]
